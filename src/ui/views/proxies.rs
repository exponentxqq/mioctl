...
pub fn render(f: &mut Frame, area: Rect, state: &AppState, table_state: &mut TableState) {
    ...
    let group_items: Vec<ListItem> = state
        ...
        .map(|(i, g)| {
            ...
            ListItem::new(Line::from(Span::styled(
                format!("{} ({})", readable_name(&g.name), g.all.len()),
                style,
            )))
        })
        .collect();
    let groups =
        List::new(group_items).block(Block::default().title("Groups").borders(Borders::RIGHT));
    ...
    let group = match state.groups.get(state.ui.selected_group_idx) {
        ...
        None => {
            f.render_widget(
                Paragraph::new("Select a group").style(Style::default().fg(T.text)),
                chunks[1],
            );
            return;
        }
    ...
    let header =
        Row::new(vec!["Name", "Type", "Latency"]).style(Style::default().fg(T.text_secondary));
    ...
    let rows: Vec<Row> = visible_nodes
        ...
        .map(|(orig_idx, name)| {
            ...
            Row::new(vec![
                format!("{}{}", prefix, readable_name(name)),
                ptype.into(),
                delay,
            ])
            .style(style)
    ...
    let title = if state.ui.search_mode {
        format!(
            "Nodes - {} [search: /{}]",
            group.name, state.ui.search_query
        )
    ...
    let visible_idx = visible_nodes
        .iter()
        .position(|(orig_idx, _)| *orig_idx == state.ui.selected_node_idx);
