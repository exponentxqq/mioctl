...
pub fn render(f: &mut Frame, area: Rect, state: &AppState, spark: &TrafficSpark) {
    ...
    let mode_val = card_value(
        &format!("{:?}", state.proxy_mode),
        LoadingKind::SwitchMode,
        state,
    );
    ...
    let tun_val = card_value(
        if tun_enabled { "ON" } else { "OFF" },
        LoadingKind::ToggleProxy,
        state,
    );
    card(f, cards2[0], "TUN", &tun_val, tun_color);

    let sp_color = if state.system_proxy_enabled {
        T.green
    } else {
        T.surface
    };
    let sp_val = card_value(
