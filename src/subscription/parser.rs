...
pub fn parse_yaml(content: &str) -> Result<Vec<ParsedNode>, String> {
    ...
    let items = match proxies_arr {
        ...
        None => {
            ...
            struct YamlProxies { proxies: Option<Vec<serde_json::Value>> }
    ...
    let nodes: Vec<ParsedNode> = items.iter()
...
fn parse_proxy_value(val: &serde_json::Value) -> Option<ParsedNode> {
    ...
    let server = obj.get("server").and_then(|v| v.as_str()).unwrap_or("").to_string();
    // Port can be string or number in YAML
    let port = obj.get("port").and_then(|v| {
        if let Some(n) = v.as_u64() { return Some(n as u16); }
        if let Some(s) = v.as_str() { return s.parse().ok(); }
        None
    }).unwrap_or(443);
    let cipher = obj.get("cipher").and_then(|v| v.as_str()).map(|s| s.to_string());
    let password = obj.get("password").and_then(|v| v.as_str()).map(|s| s.to_string());
    let uuid = obj.get("uuid").and_then(|v| v.as_str()).map(|s| s.to_string());
    let alter_id = obj.get("alterId").and_then(|v| v.as_u64()).map(|n| n as u16);
    let network = obj.get("network").and_then(|v| v.as_str()).map(|s| s.to_string());
    let ws_opts = obj.get("ws-opts").cloned();
    let sni = obj.get("sni").and_then(|v| v.as_str()).map(|s| s.to_string());
    let skip_cert_verify = obj.get("skip-cert-verify").and_then(|v| v.as_bool());
    let udp = obj.get("udp").and_then(|v| v.as_bool());

    Some(ParsedNode {
        name, node_type, server, port,
        cipher, password, uuid, alter_id,
        network, ws_opts, sni, skip_cert_verify, udp,
    })
}
...
pub fn parse_uri_list(content: &str) -> Result<Vec<ParsedNode>, String> {
    ...
    for line in content.trim().lines() {
        ...
        if let Some(node) = parse_single_uri(line) { nodes.push(node); }
    }
    Ok(nodes)
}
...
fn parse_shadowsocks(url: &Url) -> Option<ParsedNode> {
    ...
    Some(ParsedNode {
        ...
        cipher: Some(cipher), password: Some(password),
        uuid: None, alter_id: None, network: None, ws_opts: None,
        sni: None, skip_cert_verify: None, udp: Some(true),
    })
}
...
fn parse_vmess(uri: &str) -> Option<ParsedNode> {
    ...
    #[derive(serde::Deserialize)]
    struct V { ps: Option<String>, add: Option<String>, port: Option<u16>,
        id: Option<String>, aid: Option<u16>, net: Option<String>,
        #[serde(rename="type")] security: Option<String>,
        host: Option<String>, path: Option<String>, tls: Option<String>, sni: Option<String> }
    let cfg: V = serde_json::from_str(&json_str).ok()?;
    let ws = if cfg.net.as_deref() == Some("ws") {
        Some(serde_json::json!({"path":cfg.path.unwrap_or("/".into()),"headers":{"Host":cfg.host.unwrap_or_default()}}))
    } else { None };
    Some(ParsedNode {
        name: cfg.ps.unwrap_or_default(), node_type: "vmess".into(),
        server: cfg.add?, port: cfg.port.unwrap_or(443),
        cipher: cfg.security, password: None, uuid: cfg.id, alter_id: cfg.aid,
        network: cfg.net, ws_opts: ws, sni: cfg.sni,
        skip_cert_verify: cfg.tls.map(|t| t=="tls"), udp: Some(true),
    })
}

fn parse_trojan(url: &Url) -> Option<ParsedNode> {
    let mut sni = None;
    for (k, v) in url.query_pairs() { if k == "sni" { sni = Some(v.to_string()); } }
    Some(ParsedNode {
        name: url.fragment().unwrap_or("").into(),
        node_type: "trojan".into(),
        server: url.host_str()?.into(),
        port: url.port().unwrap_or(443),
        cipher: None, password: Some(url.username().into()),
        uuid: None, alter_id: None, network: None, ws_opts: None,
        sni, skip_cert_verify: None, udp: Some(true),
    })
}
...
mod tests {
    ...
    fn test_detect_yaml_clash() {
        assert!(matches!(detect_format("mixed-port: 7897\nproxies:"), SubscriptionFormat::Yaml));
    }
    #[test] fn test_detect_yaml_simple() {
        assert!(matches!(detect_format("proxies:\n  - name: t\n    type: ss"), SubscriptionFormat::Yaml));
    }
    #[test] fn test_detect_plain_uri() {
        assert!(matches!(detect_format("ss://Y2hhY2hhMjAtaWV0Zi1wb2x5MTMwNTpwYXNzd29yZA@1.2.3.4:8388#T"), SubscriptionFormat::PlainUri));
    }
    #[test]
...
"#;
    ...
    #[test] fn test_parse_yaml_empty() { assert!(parse_yaml("proxies: []\n").unwrap().is_empty()); }
    #[test]
    ...
    #[test] fn test_parse_trojan() {
        let n = parse_uri_list("trojan://pass@t.example.com:443?sni=example.com#TN").unwrap();
        assert_eq!(n.len(), 1);
        assert_eq!(n[0].sni.as_deref(), Some("example.com"));
    }
    #[test] fn test_parse_multiline() {
        let n = parse_uri_list("ss://YWVzLTI1Ni1nY206dGVzdDEyMw@1.2.3.4:8388#A\ntrojan://p@5.6.7.8:443#B\n\n").unwrap();
        assert_eq!(n.len(), 2);
    }
    #[test] fn test_parse_base64() {
