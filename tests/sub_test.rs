...
async fn test_real_subscription_parse() {
    ...
    let content = mioctl::subscription::fetcher::fetch_subscription(url).await.expect("fetch failed");
    ...
    match nodes {
        Ok(nodes) => {
            ...
            for (i, n) in nodes.iter().enumerate() {
                eprintln!("{:3}. {:40} | {:8} | {}:{}", i+1, n.name, n.node_type, n.server, n.port);
            }
        }
        ...
        Ok(val) => {
            ...
            if let Some(mapping) = val.as_mapping() {
                for k in mapping.keys() {
                    eprintln!("  key: {:?} = {:?} type", k, mapping.get(k).map(|v| {
                        if v.is_sequence() { format!("sequence[{}]", v.as_sequence().unwrap().len()) }
                        else if v.is_mapping() { format!("mapping") }
                        else { format!("scalar") }
                    }));
                }
            }
